...
import org.molgenis.vcf.inheritance.genemapper.model.*;
...
public class GenemapConverter {
  ...
  public static void run(
      @Nullable Path inputOmim,
      @Nullable Path inputCgd,
      Path inputHpo,
      @Nullable Path inputIncompletePenetrance,
      Path output) {
  ...
  private static List<CgdLine> readCgdFile(Path inputCgd) {
    ...
    try (InputStream fileStream = new FileInputStream(inputCgd.toFile());
        ...
        Reader reader = new BufferedReader(new InputStreamReader(gzipStream, UTF_8))) {
  ...
  private static boolean isMissingGeneException(CsvException csvException) {
    ...
    if (csvException instanceof CsvRequiredFieldEmptyException csvRequiredFieldEmptyException) {
  ...
  private static void addOmimInheritanceModes(
      List<OmimLine> omimLines,
      Map<String, Set<String>> omimHpoMapping,
      List<IncompletePenetranceLine> incompletePenetrance,
      Map<String, GeneInheritanceValue> geneInheritanceValues) {
      ...
      if (!omimLine.getPhenotypes().isEmpty()) {
        ...
        geneInheritanceValues.put(
            ...
            GeneInheritanceValue.builder()
                ...
                .isIncompletePenetrance(
                    isIncompletePenetrance(omimLine.getGene(), incompletePenetrance))
                .build());
      }
    }
  }

  private static void addCgdInheritanceModes(
      List<CgdLine> cgdLines,
      List<IncompletePenetranceLine> incompletePenetrance,
      Map<String, GeneInheritanceValue> geneInheritanceValues) {
    for (CgdLine cgdLine : cgdLines) {
      ...
      GeneInheritanceValue inheritance =
          GeneInheritanceValue.builder()
              ...
              .inheritanceModes(inheritanceModes)
              .isIncompletePenetrance(
                  isIncompletePenetrance(cgdLine.getGene(), incompletePenetrance))
              .build();
      geneInheritanceValues.put(cgdLine.getGene(), inheritance);
    }
  }

  private static boolean isIncompletePenetrance(
      String gene, List<IncompletePenetranceLine> incompletePenetrance) {
    for (IncompletePenetranceLine incompletePenetranceLine : incompletePenetrance) {
      if (incompletePenetranceLine.getGene().equals(gene)
          && incompletePenetranceLine.getSource().equals("EntrezGene")) {
        return true;
      }
    }
    return false;
  ...
  private static Set<HpoInheritanceMode> convertoToHpoBasedInheritance(
    ...
    phenotypes.stream()
        ...
        .forEach(
            phenotype ->
                Optional.ofNullable(omimHpoMapping.get(phenotype.getOmimId()))
                    .orElseThrow(() -> new NoSuchElementException(phenotype.getOmimId()))
                    .forEach(
                        hpo ->
                            result.add(
  ...
  private static void writeToFile(
    ...
    try (CSVWriter writer =
        new CSVWriter(
            new FileWriter(output.toString(), UTF_8),
            '\t',
